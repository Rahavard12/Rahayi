...
export default function Home() {
  return <main>
    <header className="header"><div className="container nav"><a className="logo" href="/">Rahayi<span>رهایی</span></a><nav><a href="#latest">آخرین مطالب</a><a href="#analysis">تحلیل</a><a href="#about">درباره رهایی</a></nav><button>جست‌وجو</button></div></header>
    ...
    <section id="latest" className="section container"><div className="sectionHead"><div><p className="eyebrow">منتخب سردبیر</p><h2>آخرین مطالب</h2></div></div><div className="posts">{articles.map((p,i)=><article className="post" key={p.slug}><div className="number">0{i+1}</div><span>{p.category}</span><h3>{p.title}</h3><p>{p.excerpt}</p><a href={`/article/${p.slug}`}>ادامه مطلب ←</a></article>)}</div></section>
